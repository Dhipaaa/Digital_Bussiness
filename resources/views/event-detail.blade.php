    ...
    <main class="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div class="lg:col-span-1">
            <div class="sticky top-32">
                @php
                $posterUrl = ($event->poster_path && \Illuminate\Support\Facades\Storage::disk('public')->exists($event->poster_path))
                ? asset('storage/' . $event->poster_path)
                : 'https://placehold.co/200x600';
                @endphp

                <img src="{{ $posterUrl }}" alt="{{ $event->title }} Poster"
                ...
                <div class="mt-8 p-6 bg-white rounded-3xl border border-slate-100 shadow-sm">
                    ...
                    <div class="flex items-center gap-4">
                        <div
                            ...
                            {{ strtoupper(substr($event->organizer ?? 'ORG', 0, 2)) }}
                        </div>
                        <div>
                            <p class="font-bold text-slate-800">{{ $event->organizer ?? 'Organizer' }}</p>
